...
y_train_1 = regressor.predict(X_train_1).reshape((-1, 1))
y_test_1 = regressor.predict(X_test_1).reshape((-1, 1))

# Training model 2
# Importing the dataset for model 2
filepath2 = os.path.join(os.path.dirname(__file__), 'data/filtered_RB-4X.csv')
...
# Predicting the result for next phase
y_train_2 = regressor.predict(X_train_2).reshape((-1, 1))
y_test_2 = regressor.predict(X_test_2).reshape((-1, 1))

# Training model 3
# Importing the dataset for model 3
filepath3 = os.path.join(os.path.dirname(__file__), 'data/filtered_RB-1X.csv')
...
# Predicting the result for next phase
y_train_3 = regressor.predict(X_train_3).reshape((-1, 1))
y_test_3 = regressor.predict(X_test_3).reshape((-1, 1))
